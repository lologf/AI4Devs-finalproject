...
- **RF-2.3** Cada vencimiento de una tarea recurrente es una **ocurrencia** con estado: `pending`, `done`, `skipped`, `overdue`. Una ocurrencia pendiente queda `overdue` cuando termina su día sin resolverse: se destaca en la web y aparece en el digest, sin avisos inmediatos adicionales. Al archivar una tarea, su ocurrencia abierta (`pending` u `overdue`) queda `cancelled` y se cancelan sus avisos pendientes.
...
- **RF-4.2** Antelaciones configurables por elemento (p. ej. 30 días + 7 días + 1 día). Por defecto: 7 días y 1 día para vencimientos; 1 día y 2 horas para citas. Los recordatorios se programan por adelantado, al crear o cambiar el elemento. Si al crearlo o moverlo alguna antelación ya ha pasado, pero el elemento aún no ha llegado a su fecha, se envía un único aviso inmediato. Si ya la ha pasado, no hay aviso inmediato: solo se aplica su flujo de estado (RF-4.3). En los vencimientos de día completo, las antelaciones son en días y el aviso sale a la hora del digest de cada miembro.
